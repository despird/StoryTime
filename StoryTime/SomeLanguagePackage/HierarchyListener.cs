...
namespace SOME.SomeLanguageService
{
    ...
    internal class HierarchyListener : IVsHierarchyEvents, IDisposable
    {
        ...
        public int OnItemAdded(uint itemidParent, uint itemidSiblingPrev, uint itemidAdded)
        {
            ...
            Debug.WriteLine("\n\tOnItemAdded\n");
            string name;
            if (!IsSomeFile(itemidAdded, out name))
            {
                return VSConstants.S_OK;
            }
        ...
        private bool IsSomeFile(uint itemId, out string canonicalName)
        {
            ...
            return (0 == string.Compare(extension, SomeLanguageService.LANGUAGE_EXT1, StringComparison.OrdinalIgnoreCase)
                || 0 == string.Compare(extension, SomeLanguageService.LANGUAGE_EXT2, StringComparison.OrdinalIgnoreCase));
        }
        ...
        private void InternalScanHierarchy(uint itemId)
        {
            ...
            while (VSConstants.VSITEMID_NIL != currentItem)
            {
                ...
                string itemName;
                if ((null != onItemAdded) && IsSomeFile(currentItem, out itemName))
                {
