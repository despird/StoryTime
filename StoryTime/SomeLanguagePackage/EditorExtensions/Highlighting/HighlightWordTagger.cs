    ...
    internal class HighlightWordTagger : ITagger<HighlightWordTag>
    {
        ...
        static bool WordExtentIsValid(SnapshotPoint currentRequest, TextExtent word)
        {
            return word.IsSignificant
                ...
                && validSomeToken(currentRequest, word);
        }

        static bool validSomeToken(SnapshotPoint currentRequest, TextExtent word)
        {
            string wordStr = word.Span.GetText();
            string lineText = word.Span.Start.GetContainingLine().GetText();

            return validSomeTokenContent(lineText, wordStr);
        }

        static bool validSomeTokenContent(string content, string word)
        {
            SomeTokenList tokenList = new SomeTokenList();
            SomeCompiler.lexline(tokenList, 0, content);
            for (SomeToken token = tokenList.First; token != null; token = token.Next)
            {
                switch (token.Type)
                {
                    case SomeTokenType.Ident:
                        if (token.TokenString == word)
                        {
                            return true;
                        }break;
                    case SomeTokenType.ParenPair:
                        string inside = token.TokenString.Substring(1, token.TokenString.Length - 2);
                        if(validSomeTokenContent(inside, word))
                        {
                            return true;
                        }break;
                }
                if (token.TokenString == word && token.Type == SomeTokenType.Ident)
                {
                    return true;
                }
            }
