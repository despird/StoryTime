...
namespace SOME.SomeLanguageService.EditorExtensions
{
    ...
    public class Analyzer : AstWalker
    {
        ...
        {
            Define(node.Name, new SomeClassDefinition(node));
        ...
        public override bool Walk(IronPython.Compiler.Ast.FunctionDefinition node)
        {
            ...
            Define(node.Name, new SomeFunctionDefinition(node));
        ...
        private void PushScope(FunctionDefinition func)
        {
            PushScope(new FunctionScope(module, current, func));
        }
        private void PushScope(ClassDefinition cls)
        {
