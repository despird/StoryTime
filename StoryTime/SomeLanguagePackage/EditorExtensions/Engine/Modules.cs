...
namespace SOME.SomeLanguageService.EditorExtensions
{
    ...
    public class Module
    {
        ...
        public InferredClass GetClass(ClassDefinition cls)
        {
        ...
        public IList<Declaration> GetAttributesAt(int line, int column)
        {
            ...
            if (Locate(line, column, out node, out scope))
            {
                ...
                else if (node != null && !(node is ConstantExpression))
                {
    ...
    public class Modules
    {
        ...
        InferredSomeType builtins = new InferredSomeType(typeof(IronPython.Modules.Builtin));

        InferredSomeType inferredInt = new InferredSomeType(typeof(IronPython.Runtime.Operations.IntOps));
        InferredSomeType inferredBgi = new InferredSomeType(typeof(IronPython.Runtime.Operations.LongOps));
        InferredSomeType inferredCpx = new InferredSomeType(typeof(IronPython.Runtime.Operations.ComplexOps));
        InferredSomeType inferredDbl = new InferredSomeType(typeof(IronPython.Runtime.Operations.FloatOps));
        InferredSomeType inferredStr = new InferredSomeType(typeof(IronPython.Runtime.Operations.StringOps));
        InferredSomeType inferredTpl = new InferredSomeType(typeof(IronPython.Runtime.Tuple));
        InferredSomeType inferredLst = new InferredSomeType(typeof(IronPython.Runtime.List));
        InferredSomeType inferredDct = new InferredSomeType(typeof(IronPython.Runtime.Dict));
        ...
        private bool TryImportExternal(string baseName, string name, out Inferred inferred)
        {
            ...
            string file = Path.Combine(path, name + ".sms");
        ...
        private void LoadBuiltins()
        {
            ...
            foreach (PythonModuleAttribute pma in attributes)
            {
                ...
                {
                    global.AddSomeType(pma.name, pma.type);
                }
            }
            global.AddSomeType("sys", typeof(IronPython.Runtime.SystemState));
            global.AddSomeType("clr", typeof(IronPython.Modules.ClrModule));
        }
    }
